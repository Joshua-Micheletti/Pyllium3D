...
def place_light(self, name, x, y, z):
    if name in self.lights:
        if (
            ...
            or y != self.light_positions[self.lights[name] * 3 + 1]
            or z != self.light_positions[self.lights[name] * 3 + 2]
        ):
...
def calculate_model_matrix(self, name):
    ...
    self.model_matrices[name] = glm.rotate(
        ...
        glm.radians(self.rotations[name].x),
        glm.vec3(1.0, 0.0, 0.0),
    )
    self.model_matrices[name] = glm.rotate(
        self.model_matrices[name],
        glm.radians(self.rotations[name].y),
        glm.vec3(0.0, 1.0, 0.0),
    )
    self.model_matrices[name] = glm.rotate(
        self.model_matrices[name],
        glm.radians(self.rotations[name].z),
