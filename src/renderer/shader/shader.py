...
class Shader:
    ...
    def _check_uniforms(self):
        ...
        if glGetUniformLocation(self.program, "skybox_view") != -1:
            self.uniforms["skybox_view"] = glGetUniformLocation(
                self.program, "skybox_view"
            )

        # projection matrix uniform (mat4)
        if glGetUniformLocation(self.program, "projection") != -1:
            self.uniforms["projection"] = glGetUniformLocation(
        ...
        if glGetUniformLocation(self.program, "light_ambient") != -1:
            self.uniforms["light_ambient"] = glGetUniformLocation(
                self.program, "light_ambient"
            )
        # light diffuse component
        if glGetUniformLocation(self.program, "light_diffuse") != -1:
            self.uniforms["light_diffuse"] = glGetUniformLocation(
                self.program, "light_diffuse"
            )
        # light specular component
        if glGetUniformLocation(self.program, "light_specular") != -1:
            self.uniforms["light_specular"] = glGetUniformLocation(
                self.program, "light_specular"
            )
        ...
        if glGetUniformLocation(self.program, "screen_texture") != -1:
            self.uniforms["screen_texture"] = glGetUniformLocation(
                self.program, "screen_texture"
            )

        if glGetUniformLocation(self.program, "blurred_texture") != -1:
            self.uniforms["blurred_texture"] = glGetUniformLocation(
                self.program, "blurred_texture"
            )

        if glGetUniformLocation(self.program, "depth_texture") != -1:
            self.uniforms["depth_texture"] = glGetUniformLocation(
                self.program, "depth_texture"
            )
        ...
        if glGetUniformLocation(self.program, "user_distance") != -1:
            self.uniforms["user_distance"] = glGetUniformLocation(
                self.program, "user_distance"
            )
            self.user_uniforms["user_distance"] = 10
        if glGetUniformLocation(self.program, "user_range") != -1:
            self.uniforms["user_range"] = glGetUniformLocation(
                self.program, "user_range"
            )
            self.user_uniforms["user_range"] = 5

        if glGetUniformLocation(self.program, "user_parameter_0") != -1:
            self.uniforms["user_parameter_0"] = glGetUniformLocation(
                self.program, "user_parameter_0"
            )
            self.user_uniforms["user_parameter_0"] = 1

        if glGetUniformLocation(self.program, "user_parameter_1") != -1:
            self.uniforms["user_parameter_1"] = glGetUniformLocation(
                self.program, "user_parameter_1"
            )
            self.user_uniforms["user_parameter_1"] = 1

        if glGetUniformLocation(self.program, "user_parameter_2") != -1:
            self.uniforms["user_parameter_2"] = glGetUniformLocation(
                self.program, "user_parameter_2"
            )
            self.user_uniforms["user_parameter_2"] = 1

        if glGetUniformLocation(self.program, "user_parameter_3") != -1:
            self.uniforms["user_parameter_3"] = glGetUniformLocation(
                self.program, "user_parameter_3"
            )
        ...
        if glGetUniformLocation(self.program, "light_color") != -1:
            self.uniforms["light_color"] = glGetUniformLocation(
                self.program, "light_color"
            )

        if glGetUniformLocation(self.program, "light_strength") != -1:
            self.uniforms["light_strength"] = glGetUniformLocation(
                self.program, "light_strength"
            )
        ...
        if glGetUniformLocation(self.program, "light_colors") != -1:
            self.uniforms["light_colors"] = glGetUniformLocation(
                self.program, "light_colors"
            )

        if glGetUniformLocation(self.program, "light_strengths") != -1:
            self.uniforms["light_strengths"] = glGetUniformLocation(
                self.program, "light_strengths"
            )

        if glGetUniformLocation(self.program, "lights_count") != -1:
            self.uniforms["lights_count"] = glGetUniformLocation(
                self.program, "lights_count"
            )

        if glGetUniformLocation(self.program, "cube_matrices") != -1:
            self.uniforms["cube_matrices"] = glGetUniformLocation(
                self.program, "cube_matrices"
            )
        ...
        if glGetUniformLocation(self.program, "irradiance_map") != -1:
            self.uniforms["irradiance_map"] = glGetUniformLocation(
                self.program, "irradiance_map"
            )

        if glGetUniformLocation(self.program, "reflection_map") != -1:
            self.uniforms["reflection_map"] = glGetUniformLocation(
                self.program, "reflection_map"
            )

        if glGetUniformLocation(self.program, "brdf_integration") != -1:
            self.uniforms["brdf_integration"] = glGetUniformLocation(
                self.program, "brdf_integration"
            )

        if glGetUniformLocation(self.program, "src_resolution") != -1:
            self.uniforms["src_resolution"] = glGetUniformLocation(
                self.program, "src_resolution"
            )
        ...
        if glGetUniformLocation(self.program, "gaussian_kernel") != -1:
            self.uniforms["gaussian_kernel"] = glGetUniformLocation(
                self.program, "gaussian_kernel"
            )
